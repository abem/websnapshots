...
OCR_PROMPT = """Return JSON only: {"page_title": "string", "main_text": "string", "headings": ["string"], "links": ["string"], "buttons": ["string"]}"""{
  "page_title": "string",
...
GLM-4.0.V APIを使用して画像をOCR分析する。"""
    ...
    if model == "glm-4.0.v" and "glm-4v-plus" not in fallback_models:
    ...
    for try_model in fallback_models:
        try:
            print(f"GLM-4.0.VでOCR分析中... (モデル: {try_model})")
